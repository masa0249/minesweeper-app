...
def generate_minesweeper_openi(width, height):
    openi = np.zeros((height, width), dtype=int)
    return openi

# マインスイパーの盤面（フラグが立っているかどうかを管理）
def generate_minesweeper_flagi(width, height):
    flagi = np.zeros((height, width), dtype=int)
    return flagi
...
# 描画する関数
def plot_board(board, openi, flagi):
    height, width = board.shape
    fig, ax = plt.subplots(figsize=(width * 0.5, height * 0.5))
    
    # 背景色（未開け: 0 を灰色に設定）
    display_board = np.where(openi == 0, np.nan, board)  # 未開け部分を NaN に
    ax.matshow(display_board, cmap="cool", alpha=0.5, vmin=-1, vmax=8)  # 値域を調整

    for y in range(height):
        for x in range(width):
            if openi[y, x] == 0 and flagi[y, x] == 0:  # 未開けのセル
...
if "board" not in st.session_state:
    ...
    st.session_state.openi = generate_minesweeper_openi(width, height)
    st.session_state.flagi = generate_minesweeper_flagi(width, height)
    flagi = 0
...
# 表示用ボタン
if st.button("セルを開く"):
    if st.session_state.flagi[y, x] == 0 and st.session_state.openi[y, x] == 0:
        st.write(f"座標 ({x}, {y}) を開きました！")
        st.session_state.openi[y, x] = 1  # セルを開く
        plot_board(st.session_state.board, st.session_state.openi, st.session_state.flagi)
    else :
...
if st.button("フラグを立てる"):
    if st.session_state.flagi[y, x] == 0 and st.session_state.openi[y,x] == 0:
        ...
        st.session_state.flagi[y, x] = 1  # フラグを立てる
        flagi += 1
        plot_board(st.session_state.board, st.session_state.openi, st.session_state.flagi)
    else :
